...
<p align="center">
  <img src="https://img.shields.io/badge/C%2B%2B-11%20%7C%2014%20%7C%2017%20%7C%2020%20%7C%2023-00599C?style=flat&logo=cplusplus&logoColor=white" alt="C++ standards">
  <img src="https://img.shields.io/badge/level-0%20%E2%86%92%20pro-success?style=flat" alt="level">
  <img src="https://img.shields.io/badge/PRs-welcome-brightgreen?style=flat" alt="PRs welcome">
  <img src="https://img.shields.io/badge/lang-%D1%80%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9-blue?style=flat" alt="language">
</p>
...
- [✅ Чек-листы навыков по уровням](#-чек-листы-навыков-по-уровням)
- [🎤 Вопросы для собеседований](#-вопросы-для-собеседований)
...
- [🚀 Практические проекты по уровням](#-практические-проекты-по-уровням)
...
> Тонкие ошибки, которые часто допускают даже опытные разработчики.

```cpp
// АНТИ-1: Циклическая зависимость shared_ptr — утечка памяти
struct Node {
    std::shared_ptr<Node> next; // ❌ если узлы ссылаются по кругу — счётчик
                                //    никогда не дойдёт до 0
};
// ИСПРАВЛЕНИЕ: обратную ссылку делать std::weak_ptr

// АНТИ-2: Лишние копии в range-based for
for (auto x : big_vector) { ... }        // ❌ копирует каждый элемент
for (const auto& x : big_vector) { ... } // ✅ ссылка, без копий

// АНТИ-3: Ложный noexcept
std::string build() noexcept {           // ❌ внутри может бросить bad_alloc
    return std::string(1'000'000, 'x');  //    -> std::terminate при нехватке памяти
}

// АНТИ-4: Возврат ссылки на локальную переменную
const std::string& name() {
    std::string s = "tmp";
    return s;                            // ❌ висячая ссылка — UB
}

// АНТИ-5: Сравнение чисел с плавающей точкой через ==
if (a == 0.1 + 0.2) { ... }             // ❌ почти всегда false
if (std::abs(a - 0.3) < 1e-9) { ... }   // ✅ сравнение с эпсилон

// АНТИ-6: new/delete вручную вместо умных указателей
T* p = new T();                          // ❌ забудете delete = утечка
auto p = std::make_unique<T>();          // ✅ RAII

// АНТИ-7: Захват по ссылке в лямбде, переживающей scope
auto make() {
    int local = 42;
    return [&]{ return local; };         // ❌ local умрёт — висячая ссылка
}                                        // ✅ захватывайте [=] или [local]

// АНТИ-8: Модификация контейнера во время обхода
for (auto it = v.begin(); it != v.end(); ++it)
    if (cond(*it)) v.push_back(x);       // ❌ инвалидация итераторов — UB
```
...
Теория и задачи закрепляются только в реальных проектах. Ниже — по нескольку проектов на каждый уровень: что построить, какие навыки прокачаешь и на что обратить внимание. Делай их в отдельном репозитории, пиши README и тесты — это и есть будущее портфолио.

### 🟢 Уровень 1 — Базовый C++

<details>
<summary><b>1. Консольный калькулятор с парсером выражений</b></summary>

**Что делаем:** программа читает строку вроде `3 + 4 * (2 - 1)` и считает результат с учётом приоритета операций.

**Навыки:** работа со строками, рекурсивный спуск (recursive descent parser), стек, обработка ошибок ввода.

**На что обратить внимание:** раздели лексер (разбивает строку на токены) и парсер (строит дерево/считает). Не валидируй ввод «на ходу» — сначала токены, потом разбор. Добавь юнит-тесты на крайние случаи: деление на ноль, лишние скобки, пустой ввод.

</details>

<details>
<summary><b>2. Менеджер задач (To-Do) в файле</b></summary>

**Что делаем:** CRUD-приложение в консоли: добавить/удалить/отметить задачу, сохранение в текстовый или CSV-файл.

**Навыки:** `std::vector`, `std::string`, файловый ввод-вывод (`fstream`), сериализация, меню в цикле.

**На что обратить внимание:** не держи всё в `main()` — вынеси `Task` и `TaskManager` в отдельные классы и файлы. Файл загружай при старте и сохраняй при выходе. Это первый шаг к разделению логики и представления.

</details>

<details>
<summary><b>3. Игра «Угадай число» / «Виселица»</b></summary>

**Что делаем:** классическая игра с генерацией случайного числа/слова и подсчётом попыток.

**Навыки:** `<random>` (а не `rand()`!), циклы, условия, ввод пользователя, состояние игры.

**На что обратить внимание:** используй `std::mt19937` с `std::uniform_int_distribution` вместо устаревшего `rand() % n`. Вынеси игровой цикл в функцию, чтобы можно было сыграть заново без перезапуска.
...
<details>
<summary><b>4. Свой контейнер: динамический массив (аналог std::vector)</b></summary>

**Что делаем:** шаблонный класс `Vector<T>` с `push_back`, `operator[]`, итераторами, ростом ёмкости ×2.

**Навыки:** шаблоны, RAII, правило пяти, `std::allocator` / `placement new`, move-семантика, итераторы.

**На что обратить внимание:** разделяй размер (`size`) и ёмкость (`capacity`). Реализуй корректные copy/move конструкторы и присваивания. Проверь себя через `valgrind` или AddressSanitizer — утечек и двойного освобождения быть не должно.

</details>

<details>
<summary><b>5. JSON-парсер</b></summary>

**Что делаем:** библиотека, которая парсит JSON-строку в дерево объектов и умеет сериализовать обратно.

**Навыки:** `std::variant`, рекурсия, `std::unordered_map`, обработка ошибок, проектирование API.

**На что обратить внимание:** значение JSON — это `std::variant` из null/bool/number/string/array/object. Сначала спроектируй тип `JsonValue`, потом парсер. Сравни своё API с nlohmann/json — поймёшь, как делают «по-взрослому».

</details>

<details>
<summary><b>6. Многопоточный загрузчик файлов / пул задач</b></summary>

**Что делаем:** `ThreadPool`, который принимает задачи (`std::function`) и выполняет их на N потоках.

**Навыки:** `std::thread`, `std::mutex`, `std::condition_variable`, `std::future`, очередь задач.

**На что обратить внимание:** очередь задач защищается мьютексом, потоки спят на `condition_variable` пока нет работы. Аккуратно реализуй остановку пула (флаг + `notify_all` + `join`). Протестируй под ThreadSanitizer на гонки.
...
<details>
<summary><b>7. HTTP-сервер с нуля (на сокетах)</b></summary>

**Что делаем:** сервер, который слушает порт, парсит HTTP-запросы и отдаёт статические файлы / простые роуты.

**Навыки:** сокеты (POSIX/Winsock или Boost.Asio), асинхронность, парсинг протокола, многопоточность/epoll.

**На что обратить внимание:** начни с синхронной однопоточной версии, потом добавь пул потоков или асинхронный event loop. Не доверяй вводу из сети — это untrusted data: проверяй длины, таймауты, размер тела. Отличный проект для понимания, как работает веб «под капотом».

</details>

<details>
<summary><b>8. Мини-интерпретатор / стековая виртуальная машина</b></summary>

**Что делаем:** язык с переменными, арифметикой, условиями и циклами: лексер → парсер (AST) → байткод → стековая VM.

**Навыки:** `std::variant`, паттерн Visitor, проектирование AST, управление памятью, оптимизация.

**На что обратить внимание:** раздели фронтенд (лексер+парсер) и бэкенд (VM). AST обходи через `std::visit`. Это серьёзный проект — отличная строчка в резюме и глубокое понимание того, как устроены языки.

</details>

<details>
<summary><b>9. Игровой движок 2D / физический симулятор</b></summary>

**Что делаем:** рендер через SDL2/SFML, игровой цикл с фиксированным шагом, простая физика (гравитация, столкновения AABB).

**Навыки:** ECS-архитектура, управление ресурсами, фиксированный timestep, профилирование, оптимизация кэша.

**На что обратить внимание:** отдели логику от рендера, используй data-oriented подход (массивы компонентов, а не дерево объектов) — почувствуешь разницу в производительности. Профилируй (`perf`, VTune) узкие места.
...
<details>
<summary><b>10. Lock-free структура данных (очередь/стек)</b></summary>

**Что делаем:** потокобезопасная очередь без блокировок на атомиках, корректная по memory ordering.

**Навыки:** `std::atomic`, `memory_order`, CAS-циклы, ABA-проблема, hazard pointers.

**На что обратить внимание:** это сложно — сначала прочитай Anthony Williams «C++ Concurrency in Action». Проверяй корректность стресс-тестами и санитайзерами; помни про ABA-проблему. Не используй такое в продакшене без крайней необходимости — обычный мьютекс часто быстрее и проще.

</details>

<details>
<summary><b>11. Аллокатор памяти (pool / arena)</b></summary>

**Что делаем:** кастомный аллокатор, совместимый с `std::pmr` или Allocator-концептом, с пулами фиксированного размера.

**Навыки:** `std::pmr::memory_resource`, выравнивание, фрагментация, бенчмарки.

**На что обратить внимание:** сравни производительность с дефолтным `new` через бенчмарки (Google Benchmark). Поддержи выравнивание (`alignof`). Pool-аллокатор почти всегда быстрее для множества мелких однотипных объектов.

</details>

<details>
<summary><b>12. Распределённое key-value хранилище</b></summary>

**Что делаем:** сетевая БД в памяти с репликацией, WAL (write-ahead log) и простым консенсусом (Raft).

**Навыки:** сеть, персистентность, конкурентность, распределённые системы, тестирование под нагрузкой.

**На что обратить внимание:** это проект на месяцы — разбей на этапы: однонодовое KV → персистентность через WAL → репликация → консенсус. Каждый этап заслуживает отдельных тестов. Топовый проект для senior-портфолио.

</details>

> 💡 **Как вести проекты:** отдельный git-репозиторий на каждый проект, осмысленные коммиты, README с описанием и инструкцией сборки (CMake), юнит-тесты (GoogleTest/Catch2), CI на GitHub Actions. Именно так это выглядит в реальной работе.


## ❓ FAQ — частые вопросы

<details>
<summary><b>Какой компилятор и ОС выбрать для старта?</b></summary>

Любой современный из GCC, Clang или MSVC подойдёт. На Windows проще всего поставить Visual Studio (MSVC) или MSYS2/MinGW. На Linux/macOS — GCC или Clang из пакетного менеджера. Начинайте с C++20, включайте `-Wall -Wextra`.
</details>

<details>
<summary><b>Нужна ли математика для C++?</b></summary>

Для большинства прикладных задач достаточно школьной. Серьёзная математика нужна в специализациях: графика/геймдев (линейная алгебра), HPC и ML (численные методы), криптография. Но начать учить язык можно без неё.
</details>

<details>
<summary><b>C++ или Rust в 2026?</b></summary>

Это не «или-или». C++ доминирует в геймдеве, embedded, легаси-системах, финансах и там, где нужна совместимость с огромной кодовой базой. Rust привлекателен безопасностью памяти для новых системных проектов. Знание C++ остаётся очень востребованным; идеи Rust полезно понимать.
</details>

<details>
<summary><b>Сколько времени нужно, чтобы выучить C++?</b></summary>

До уверенного Junior-уровня — около 6–12 месяцев регулярной практики. До Middle — ещё 1–2 года реального опыта. C++ глубок: эксперты учатся постоянно. Главное — не зубрить, а писать код каждый день и читать чужой.
</details>

<details>
<summary><b>Какой стандарт учить — старый или новый?</b></summary>

Учите сразу современный (C++17/20): он проще, безопаснее и читаемее. Старые практики (сырые `new`/`delete`, C-строки) изучайте только чтобы понимать легаси-код, но не пишите так сами.
</details>

<details>
<summary><b>Обязательно ли решать олимпиадные задачи?</b></summary>

Нет, но они отлично прокачивают алгоритмическое мышление и пригодятся на собеседованиях в крупные компании. Для прикладной разработки важнее проекты, дизайн и понимание инструментов. Баланс индивидуален.
</details>
---
...
## ✅ Чек-листы навыков по уровням

> Отмечайте пункты по мере освоения. Переходите на следующий уровень, только когда уверенно закрыли 80%+ текущего.

### Уровень 1 — Junior готов, если умеете:

- [ ] Объявлять переменные с правильными типами и всегда их инициализировать
- [ ] Писать функции, понимать передачу по значению / по ссылке / по const-ссылке
- [ ] Работать с `std::string` и `std::vector` (добавление, обход, поиск)
- [ ] Применять базовые алгоритмы STL: `sort`, `find`, `count_if`, `accumulate`
- [ ] Понимать разницу между указателем и ссылкой, проверять указатели на `nullptr`
- [ ] Компилировать с `-Wall -Wextra` и исправлять предупреждения
- [ ] Пользоваться range-based for и `auto`

### Уровень 2 — Middle готов, если умеете:

- [ ] Проектировать классы: инкапсуляция, конструкторы, `explicit`, const-методы
- [ ] Применять наследование и виртуальные функции, помнить про `virtual ~`
- [ ] Выбирать правильный умный указатель (`unique_ptr`/`shared_ptr`/`weak_ptr`)
- [ ] Писать простые шаблоны функций и классов
- [ ] Обрабатывать ошибки через исключения и `std::optional`
- [ ] Выбирать подходящий STL-контейнер под задачу
- [ ] Понимать инвалидацию итераторов

### Уровень 3 — Senior готов, если умеете:

- [ ] Объяснять move-семантику и категории значений, применять `std::move`/`std::forward`
- [ ] Писать потокобезопасный код: `mutex`, `lock_guard`, `condition_variable`, `atomic`
- [ ] Применять RAII для любых ресурсов, писать ScopeGuard
- [ ] Использовать концепты (C++20) и `if constexpr`
- [ ] Понимать NRVO/copy elision и не мешать им
- [ ] Настраивать проект через CMake с разными конфигурациями
- [ ] Профилировать и искать UB санитайзерами

### Уровень 4 — Expert, если умеете:

- [ ] Писать cache-friendly код (SoA, выравнивание, ложное разделение)
- [ ] Применять ranges и `std::format` (C++20), корутины (C++20/23)
- [ ] Понимать модель памяти и `memory_order`, писать lock-free структуры
- [ ] Глубоко владеть выбранной специализацией (gamedev / HPC / embedded / компиляторы)
- [ ] Читать ассемблер в Compiler Explorer и понимать, что генерирует компилятор
---

## 🎤 Вопросы для собеседований

> Краткие ответы для самопроверки. На реальном собеседовании важно не только знать ответ, но и уметь объяснить *почему*.

### Junior

<details>
<summary><b>В чём разница между указателем и ссылкой?</b></summary>

Ссылка — это псевдоним существующего объекта: её нельзя «перепривязать» и она не может быть «нулевой». Указатель — отдельная переменная, хранящая адрес; его можно переназначить, обнулить (`nullptr`) и над ним возможна арифметика. Ссылку нужно инициализировать сразу при объявлении.
</details>

<details>
<summary><b>Что выведет <code>sizeof</code> для разных типов и почему?</b></summary>

`sizeof` возвращает размер типа в байтах на этапе компиляции. Для встроенных типов он платформозависим (но `char` всегда 1). Для структур размер может быть больше суммы полей из-за **выравнивания** (padding) — компилятор вставляет пустые байты, чтобы поля лежали по «удобным» адресам.
</details>

<details>
<summary><b>Чем <code>++i</code> отличается от <code>i++</code>?</b></summary>

`++i` (префиксный) увеличивает значение и возвращает уже новое. `i++` (постфиксный) возвращает старое значение, а потом увеличивает — для этого создаётся временная копия. Для тяжёлых типов (итераторов) предпочитайте `++i`.
</details>

### Middle

<details>
<summary><b>Зачем виртуальному базовому классу виртуальный деструктор?</b></summary>

Чтобы при `delete` через указатель на базовый класс вызвался деструктор реального (производного) объекта. Без `virtual ~Base()` будет вызван только деструктор базового класса — это UB и утечка ресурсов производного. Правило: есть хоть одна виртуальная функция — делайте деструктор виртуальным.
</details>

<details>
<summary><b>Когда использовать <code>shared_ptr</code>, а когда <code>unique_ptr</code>?</b></summary>

`unique_ptr` — для единственного владельца (это 90% случаев, нулевые накладные расходы). `shared_ptr` — только когда владение действительно разделяется между несколькими сущностями; он дороже из-за атомарного счётчика ссылок. Циклы `shared_ptr` разрывайте через `weak_ptr`.
</details>

<details>
<summary><b>Что такое RAII и какие примеры в стандартной библиотеке?</b></summary>

RAII связывает владение ресурсом с временем жизни объекта: захват в конструкторе, освобождение в деструкторе — гарантированно, даже при исключении. Примеры: `std::unique_ptr` (память), `std::fstream` (файл), `std::lock_guard` (мьютекс), `std::jthread` (поток).
</details>

### Senior

<details>
<summary><b>Объясните move-семантику. Что делает <code>std::move</code>?</b></summary>

Move-семантика позволяет «украсть» внутренние ресурсы у временного объекта вместо дорогого копирования. `std::move` **сам ничего не перемещает** — это лишь приведение к rvalue-ссылке, которое разрешает компилятору выбрать перемещающую перегрузку. Реальное перемещение делает move-конструктор/оператор; если их нет — тихо выполняется копия.
</details>

<details>
<summary><b>Что такое гонка данных и как её избежать?</b></summary>

Гонка данных — одновременный доступ нескольких потоков к одной ячейке памяти, где хотя бы один доступ является записью, без синхронизации. Это UB. Избегают мьютексами (`std::lock_guard`/`std::scoped_lock`), атомарными операциями (`std::atomic`) или дизайном без разделяемого изменяемого состояния.
</details>

<details>
<summary><b>Что такое copy elision / RVO / NRVO?</b></summary>

Это оптимизация, при которой компилятор конструирует возвращаемый объект прямо в месте вызова, минуя копирование/перемещение. С C++17 RVO для prvalue — гарантирован. Поэтому пишите `return obj;`, а не `return std::move(obj);` — явный `move` мешает NRVO.
</details>

<details>
<summary><b>Чем concept лучше SFINAE?</b></summary>

Концепты (C++20) задают требования к типам декларативно и читаемо, а ошибки дают короткие и понятные («тип не удовлетворяет concept»). SFINAE через `enable_if` громоздок и порождает простыни ошибок из глубины инстанцирования. Концепты — современная замена, но SFINAE нужно знать для чтения легаси.
</details>
---

## 🗂️ Структуры данных и выбор контейнера

### Сложности операций STL-контейнеров

| Контейнер | Доступ | Поиск | Вставка | Удаление | Память |
|---|---|---|---|---|---|
| `vector` | O(1) | O(n) | O(1)* в конец | O(n) в середине | компактная |
| `deque` | O(1) | O(n) | O(1) с краёв | O(n) в середине | блоками |
| `list` | O(n) | O(n) | O(1) | O(1) | +указатели |
| `set` / `map` | — | O(log n) | O(log n) | O(log n) | дерево |
| `unordered_set` / `unordered_map` | — | O(1)** | O(1)** | O(1)** | хеш-таблица |
| `array` | O(1) | O(n) | — | — | на стеке |

\* амортизированно; \*\* в среднем, худший случай O(n) при коллизиях.

### Какой контейнер выбрать?

| Нужно | Берите |
|---|---|
| Динамический массив, доступ по индексу | `std::vector` (по умолчанию!) |
| Частые вставки/удаления с обоих концов | `std::deque` |
| Уникальные ключи с порядком | `std::set` / `std::map` |
| Быстрый поиск по ключу, порядок не важен | `std::unordered_map` |
| Очередь с приоритетом | `std::priority_queue` |
| Стек / очередь (FIFO) | `std::stack` / `std::queue` |
| Фиксированный размер, известен при компиляции | `std::array` |
| Невладеющий вид на массив | `std::span` (C++20) |

> 💡 **Правило по умолчанию:** начинайте с `std::vector`. Он почти всегда быстрее `list` даже для вставок в середину — за счёт кэш-локальности. Меняйте контейнер только когда профайлер докажет необходимость.

### Шпаргалка по `<algorithm>`

| Задача | Алгоритм |
|---|---|
| Отсортировать | `std::sort`, `std::stable_sort`, `std::partial_sort` |
| Найти элемент | `std::find`, `std::find_if`, `std::binary_search` |
| Подсчитать | `std::count`, `std::count_if` |
| Преобразовать все элементы | `std::transform` |
| Свернуть к одному значению | `std::accumulate`, `std::reduce` |
| Удалить по условию | `std::remove_if` + `erase` / `std::erase_if` (C++20) |
| Проверить условие | `std::all_of`, `std::any_of`, `std::none_of` |
| Min / max | `std::min_element`, `std::max_element`, `std::minmax` |
| Убрать дубликаты (в отсортированном) | `std::unique` |
---
